...
export default function SessionRow({ session, onToggleComplete }) {
  ...
  return (
    <div
      ...
      className={`group p-2.5 rounded-xl border transition flex flex-col gap-1.5 cursor-pointer select-none ${
        isCompleted
    ...
    >
      {/* Top Row: Checkbox + Topic title (Full line width, zero clipping) */}
      <div className="flex items-start gap-2 min-w-0">
        <button
          ...
          className="text-slate-400 hover:text-indigo-600 transition shrink-0 mt-0.5"
          aria-label={isCompleted ? "Mark session uncompleted" : "Mark session completed"}
        >
          {isCompleted ? (
      ...
      {/* Bottom Row: Icon + Subject + Duration Badge + Action Indicator */}
      <div className="flex items-center justify-between gap-1 pt-1.5 border-t border-slate-100 dark:border-slate-700/50">
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <span className="font-extrabold uppercase tracking-wider text-[9px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 shrink-0">
            {session.subject}
          </span>
        </div>

        <div className="flex items-center gap-1.5 shrink-0">
          <span className="text-[10px] font-mono font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/50 px-1.5 py-0.5 rounded shrink-0">
            {session.minutes}m
          </span>
          <ArrowRight className="w-3 h-3 text-slate-300 group-hover:text-indigo-600 transition shrink-0" />
        </div>
      </div>
    </div>
  );
