...
export default function InteractionBar({ post, onOpenComments, onSaveChange, isSaved = false }) {
  const postId = post.id || `p_${Math.abs(hashString((post.title || '') + (post.body || '')))}`;
  
  const initialLikes = (post.engagement && post.engagement.likes) || 24;
  ...
  useEffect(() => {
    try {
      ...
      const savedPosts = JSON.parse(localStorage.getItem('studyrot_saved_posts') || '[]');
      const alreadySaved = savedPosts.some((p) => p.id === postId || (p.title === post.title && p.body === post.body));
  ...
  const toggleSave = (e) => {
    ...
    try {
      let savedPosts = JSON.parse(localStorage.getItem('studyrot_saved_posts') || '[]');
      if (nextSaved) {
        const postToSave = { ...post, id: postId, savedAt: new Date().toISOString() };
        savedPosts = [postToSave, ...savedPosts.filter((p) => p.id !== postId)];
      } else {
        savedPosts = savedPosts.filter((p) => p.id !== postId && p.title !== post.title);
      }
  ...
  const handleShare = async (e) => {
    ...
    const shareTitle = `StudyRot: ${post.title}`;
    const shareText = `Check out this CBSE Class ${post.grade || 10} ${post.subject || 'NCERT'} card on StudyRot!`;
  ...
  const commentsCount =
    ...
    (post.engagement && post.engagement.seed_comment ? 1 : 3);
