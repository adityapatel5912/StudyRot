...
function MainApp() {
  ...
  const [activeModal, setActiveModal] = useState<string | null>(null); // 'my-feeds' | 'saved' | 'keys' | null
  ...
  // Check for restorable cached feed on mount
  useEffect(() => {
  ...
  const handleGenerate = async (config) => {
    ...
    try {
      ...
      if (result && Array.isArray(result.posts) && result.posts.length > 0) {
        // Compress SVG diagrams for optimal DOM performance
  ...
  return (
    <div className="w-full min-h-screen bg-[var(--off-white)] text-[var(--navy-900)] flex flex-col font-sans">
      ...
      {/* Restorable Feed Banner (if user returns to fresh session) */}
