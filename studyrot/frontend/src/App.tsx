    ...
    try {
      ...
      if (
        ...
        if (lowerText.includes('electric') || lowerText.includes('ohm') || lowerText.includes('circuit')) {
          setPosts(SAMPLE_FEEDS.electricity_10.posts);
          setError('Loaded pre-calibrated NCERT Electricity feed (add your Groq API key above for dynamic custom chapters).');
          return;
        } else if (lowerText.includes('trig') || lowerText.includes('height') || lowerText.includes('elevation')) {
          setPosts(SAMPLE_FEEDS.trigonometry_10.posts);
          setError('Loaded pre-calibrated NCERT Trigonometry feed (add your Groq API key above for dynamic custom chapters).');
          return;
